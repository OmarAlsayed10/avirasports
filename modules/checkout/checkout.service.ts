...
type CouponValidation =
  | { ok: true; discountEgp: number; couponId: string; type: 'PERCENT' | 'FIXED'; value: number }
  | { ok: false; error: string; code: string };

async function validateCoupon(code: string, subtotalEgp: number): Promise<CouponValidation> {
  const now = new Date();
  const coupon = await prisma.coupon.findUnique({ where: { code: code.toUpperCase() } });

  if (!coupon || !coupon.isActive)
    return { ok: false, error: 'Coupon code not found', code: 'COUPON_INVALID' };
  ...
  const discountEgp =
    coupon.type === 'PERCENT' ? Math.round(subtotalEgp * (couponValue / 100)) : couponValue;

  return { ok: true, discountEgp, couponId: coupon.id, type: coupon.type, value: couponValue };
}
...
export async function placeOrder(
  ...
  return { ok: true, data: { redirectTo: `/checkout/success?orderNumber=${orderNumber}`, orderNumber } };
}
...
export async function applyCoupon(
  ...
  ActionResult<{ discountEgp: number; type: 'PERCENT' | 'FIXED'; value: number }>
> {
  ...
  return { ok: true, data: { discountEgp: result.discountEgp, type: result.type, value: result.value } };
}
