...
export function ReviewForm({ productId }: ReviewFormProps) {
  ...
  if (submitted) {
    return (
      <div className={productTokens.review.successMsg}>
  ...
  return (
    <form onSubmit={handleSubmit} className={productTokens.review.form}>
      <h3 className={productTokens.review.formTitle}>{t.product.writeReview}</h3>
      ...
      <div>
        <label className={productTokens.review.label}>{t.product.reviewTitleLabel}</label>
        <input
          value={title}
          onChange={(e) => setTitle(e.target.value)}
          maxLength={120}
          placeholder={t.product.reviewTitlePlaceholder}
          className={productTokens.review.input}
        />
      </div>

      <div>
        <label className={productTokens.review.label}>{t.product.reviewBodyLabel}</label>
        <textarea
          value={body}
          ...
          className={`${productTokens.review.input} resize-none`}
        />
        <p className="text-xs text-text-secondary mt-1 text-right">{body.length} / 2000</p>
      </div>

      {error && <p className={productTokens.review.errorMsg}>{error}</p>}

      <button
        type="submit"
        disabled={isPending || body.length < 20 || rating === 0}
        className={productTokens.review.submitBtn}
      >
