...
import { createReview } from '@/modules/review/review.service';
import { useLocale } from '@/modules/_shared/i18n/i18n.context';
import { reviewFormTokens } from './review-form.tokens';
...
export function ReviewForm({ productId }: ReviewFormProps) {
  ...
  if (submitted) {
    return (
      <div className={reviewFormTokens.successMsg}>
  ...
  return (
    <form onSubmit={handleSubmit} className={reviewFormTokens.form}>
      <h3 className={reviewFormTokens.formTitle}>{t.product.writeReview}</h3>
      ...
      <div>
        <label className={reviewFormTokens.label}>{t.product.reviewTitleLabel}</label>
        <input
          value={title}
          onChange={(e) => setTitle(e.target.value)}
          maxLength={120}
          placeholder={t.product.reviewTitlePlaceholder}
          className={reviewFormTokens.input}
        />
      </div>

      <div>
        <label className={reviewFormTokens.label}>{t.product.reviewBodyLabel}</label>
        <textarea
          value={body}
          ...
          className={`${reviewFormTokens.input} resize-none`}
        />
        <p className="text-xs text-text-secondary mt-1 text-right">{body.length} / 2000</p>
      </div>

      {error && <p className={reviewFormTokens.errorMsg}>{error}</p>}

      <button
        type="submit"
        disabled={isPending || body.length < 20 || rating === 0}
        className={reviewFormTokens.submitBtn}
      >
